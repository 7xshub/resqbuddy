...
import { useCategories, useEmergenciesByCategory } from "@/hooks/use-emergencies";
...
const CategoryPage = () => {
  ...
  const { data: items = [], isLoading } = useEmergenciesByCategory(id ?? "");

  const category = categories.find((c) => c.id === id);

  if (!category && !isLoading) return <div className="p-8 text-center text-muted-foreground">Category not found</div>;
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {emergency.severity === "critical" ? "Critical" : "Standard"}
                </p>
              </div>
